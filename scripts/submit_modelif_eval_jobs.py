...
for model_info in models:
    ...
    name = f"open_instruct_eval_modelif_{model_name}_{today}"
    d['description'] = name
    d['tasks'][0]['name'] = name
    ...
    if model_info[0] == "gpt-3.5-turbo":
        ...
        #     --nr_category "Generation" "Open QA" "Brainstorm" "Rewrite" "Summarize" "Classify" "Closed QA" "Extract" \
        #     --config_dir eval/alpaca_farm/configs \
        #     --config_name gpt4_human_reference_v0 \
        ...
        d['tasks'][0]['datasets'].append({
            'mountPath': "/dataset",
            'source': {
                'beaker': "01HV2FGMNRGKT0JRG3H0DQBPEV"
            }
        })
        # 
        # change image
