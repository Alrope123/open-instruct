...
def main(args):
    ...
    prediction_dir = os.path.join(args.prediction_dir, f"{version_name}_{model_name}")
    ...
    for category in nr_category:
        ...
        if prompts == {}:
            for i, annotation in enumerate(annotations):
                if i < n_per_cat:
                    preference_key = sorted([k for k in annotation.keys() if k.startswith("preference")])[-1] if not args.preference_key else args.preference_key
    ...
    f = csv.writer(open(os.path.join(save_dir, f"{version_name}_{model_name}_{preference_key}.csv"), "w+"))
