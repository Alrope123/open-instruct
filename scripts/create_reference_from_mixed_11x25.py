...
def main(args):
    ...
    for g in ['a', 'b']:
        ...
        for dp in data:
            formatted_data[dp['category']].append({
                ...
                "dataset": f"no_robots_11x25_stronger_{dp['category']}"
            })

        with open(os.path.join(args.save_dir, f"mixed_{g}_references_11x25_stronger.json"), 'w') as f:
    ...
    with open(args.human_data_path2, 'r') as f:
    ...
    for dp in data:
        ...
        assert found, (dp['category'], dp['text'])
        output_human_data[dp['category']].append({
            'instruction': dp['text'],
            'output': human_output,
            "generator": 'human',
            "dataset": f"no_robots_11x25_{dp['category']}"
        })
    with open(os.path.join(args.save_dir, f"mixed_human_references_11x25_stronger.json"), 'w') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="/home/xinxil/open-instruct/tmp/references/no_robot_samples_for_human_annotation_11x25_stronger.jsonl",
    )
    parser.add_argument(
        "--human_data_path",
        type=str,
    ...
    parser.add_argument(
        "--human_data_path2",
        type=str,
        default="/home/xinxil/open-instruct/tmp/data/no_robots_test_data_surge.json",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
