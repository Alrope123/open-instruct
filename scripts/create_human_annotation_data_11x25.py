...
def post_process(model, output):
    ...
    if model.startswith("Meta-Llama-3"):
        ...
        assert beginning_text in output, [output]
        output = output[output.find("<|end_header_id|>") + len("<|end_header_id|>"):].strip()
    return output
...
def main(args):
    ...
    for category in nr_category:
        ...
        for model in models:
            model_outputs[model] = json.load(open(os.path.join(prediction_dir, f"{model}_references_11x25.json"), 'r'))
    ...
    for category in nr_category:
        ...
        for i, instruction_and_output in enumerate(categorical_instruction_and_outputs):
            reference_model_and_length = [(model, len(output)) for model, output in instruction_and_output['outputs'].items() if model == reference_model][0]
            models_and_lengths = [(model, len(output)) for model, output in instruction_and_output['outputs'].items() if check_generation(output) and model != reference_model]
            models_and_lengths = sorted(models_and_lengths, key=lambda x: x[1])
            # print(models_and_lengths[0][1])
            # print(type(models_and_lengths[0][1]))
            # print(reference_model_and_length[1])
            # print(type(reference_model_and_length[1]))
            # assert False
            short_models = [m_and_l[0] for m_and_l in models_and_lengths if m_and_l[1] < reference_model_and_length[1]]
            long_models = [m_and_l[0] for m_and_l in models_and_lengths if m_and_l[1] > reference_model_and_length[1]]
            reference_models = [reference_model_and_length[0]]
            size = 4
            if len(short_models) < size // 2:
                short_models = long_models[:size // 2]
                long_models = long_models[size // 2:]
            if len(long_models) < size // 2:
                long_models = short_models[-size // 2:]
                short_models = short_models[:-size // 2]
            comparison_groups = [(short_models, reference_models), (short_models, reference_models), (long_models, reference_models), (long_models, reference_models)]
            compared_models = set()
    ...
    with open(os.path.join(save_dir, "no_robot_samples_for_human_annotation_11x25_stronger.jsonl"), 'w') as f:
        for output in final_output:
            f.write(json.dumps(output))
            f.write('\n')

    write_csv(final_output, os.path.join(save_dir, "no_robot_samples_for_human_annotation_11x25_stronger.csv"))
